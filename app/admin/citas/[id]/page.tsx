...
import {
  ...
  CheckCircle2, AlertCircle, HardHat,
} from "lucide-react";
...
export default function AdminCitaDetallePage() {
  ...
  const [mecanicoLabel, setMecanicoLabel] = useState("");
  ...
  useEffect(() => {
    ...
    setMecanicoLabel(m ? `${m.nombre} ${m.apellido}` : "");
  }, [mecanicoId, mecanicos]);

  const handleGuardar = async () => {
    setSaving(true);
    setResult({});
    const res = await actualizarEstadoCita(id, estado, notasAdmin, mecanicoId || undefined);
    setResult(res);
    setSaving(false);
    if (!res.error) setCita((prev: any) => ({ ...prev, estado, notas_admin: notasAdmin, tecnico_id: mecanicoId }));
  };
  ...
  const waLink = buildCitaWALink({ ...cita, fecha_hora_fmt: formatDateTime(cita.fecha_hora) }, estado, notasAdmin);
  const waConfig = WA_CONFIG[estado];
  const tienetelefono = !!cita.perfiles?.telefono;
  const mostrarMecanico = ["confirmada", "en_proceso", "completada"].includes(estado);
  ...
  return (
    <div>
      <div className="flex items-center gap-3 mb-8">
        ...
        <div>
          <h1 className="text-2xl font-bold" style={{ fontFamily: "var(--font-heading)" }}>Gestionar Cita</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
          ...
          <CardContent className="space-y-3">
            <div className="flex items-start gap-2.5">
              ...
              <div>
                ...
                <Link href={`/admin/clientes/${cita.perfiles?.id}`} className="text-sm font-medium hover:text-primary">
                  {cita.perfiles?.nombre} {cita.perfiles?.apellido}
                </Link>
                {cita.perfiles?.telefono
                  ? <p className="text-xs text-muted-foreground">{cita.perfiles.telefono}</p>
                  : <p className="text-xs text-muted-foreground italic">Sin teléfono registrado</p>
                }
              </div>
            </div>
            <div className="flex items-start gap-2.5">
              ...
              <div>
                ...
                <p className="text-sm font-medium">{cita.vehiculos?.marca} {cita.vehiculos?.modelo} {cita.vehiculos?.anio}</p>
            ...
            <div className="flex items-start gap-2.5">
              ...
              <div>
                ...
                <a href={SUCURSAL_MAPS[cita.sucursal]} target="_blank" rel="noopener noreferrer" className="text-xs text-primary hover:underline">
                  Ver en Google Maps
                </a>
              </div>
            </div>
            ...
            {cita.notas_cliente && (
              <div className="pt-2 border-t border-border/50">
                ...
                <p className="text-sm">{cita.notas_cliente}</p>
              </div>
            )}
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-base">Actualizar Cita</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            {mostrarMecanico && mecanicos.length > 0 && (
              <div className="space-y-1.5">
                <Label className="flex items-center gap-1.5">
                  <HardHat className="w-3.5 h-3.5 text-orange-500" />
                  Mecánico responsable
                </Label>
                <input type="hidden" value={mecanicoId} />
                <Select value={mecanicoId} onValueChange={(v) => setMecanicoId(v || "")}>
                  <SelectTrigger>
                    <SelectValue placeholder="Sin asignar">
                      {mecanicoLabel || "Sin asignar"}
                    </SelectValue>
                  </SelectTrigger>
                  <SelectContent>
            ...
            <div className="space-y-1.5">
              ...
              <Textarea
                ...
                placeholder="Observaciones o mensaje para el cliente..."
                rows={3}
              />
            </div>
            ...
            <Link href={`/admin/mantenimiento/nuevo?cita=${cita.id}&vehiculo=${cita.vehiculos?.id || ""}${mecanicoId && mecanicoId !== "ninguno" ? `&tecnico=${mecanicoId}` : ""}`}>
